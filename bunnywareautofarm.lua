...
local farmSpeed = 20
local maxCoins = 40
local autoGetGun = false -- kept for compatibility but not used in lite
...
AutoSection:Slider({
    ...
    Desc = "Set to 40 (default) or 50 (Elite gamepass)",
    Step = 10,
...
AutoSection:Toggle({
    ...
    Desc = "Kills you when you reach the max coin limit (FIXED: reads server data)",
    Value = false,
...
-- ============================================================
-- FIXED: Get coin count from SERVER DATA (reliable)
-- ============================================================
local function u65()
    -- PRIMARY METHOD: Use the game's own data remote (always accurate)
        ...
        if extras then
            ...
            if getPlayerData and getPlayerData:IsA("RemoteFunction") then
                ...
                if success and data then
                    ...
                    if myData and myData.Currency then
                        return myData.Currency or 0
                    end
                end
            end
        end
    ...
    -- FALLBACK: UI scraping (if remote fails)
...
-- ============================================================
-- AUTOFARM HEARTBEAT LOOP (with fixed reset)
-- ============================================================
RunService.Heartbeat:Connect(function()
    ...
    -- FIXED: Read coins from server data (accurate)
...
Window:Notify({
    ...
    Content = "Loaded! AutoFarm (fixed reset) + AntiAFK + AntiFling",
    Duration = 5,
})
