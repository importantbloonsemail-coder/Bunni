...
-- Now using Overdrive's proven auto reset method.
-- ============================================================
...
local farmSpeed = 20
local maxCoins = 40  -- will be overridden by Elite attribute
...
AutoSection:Toggle({
    ...
    Desc = "Kills you when you reach the max coin limit (Overdrive-style)",
    Value = false,
...
-- ============================================================
local u49 = nil  -- nil means we haven't determined yet; will be set to true/false

-- Determine if we are in game or lobby
local function checkGameOrLobby()
    ...
    u49 = nil -- fallback, will be determined later
end
checkGameOrLobby()

local function u65()
    -- Use exact same paths as Overdrive
    if u49 == false then
        -- In game
        local gui = LocalPlayer.PlayerGui
        if gui then
...
-- Override maxCoins with Elite attribute
local function getMaxCoins()
    local elite = LocalPlayer:GetAttribute("Elite")
    if elite then
        return 50
    else
        return maxCoins  -- user slider value (40 or 50)
    end
end
...
-- ============================================================
-- AUTOFARM HEARTBEAT LOOP (Overdrive-style reset)
-- ============================================================
RunService.Heartbeat:Connect(function()
    ...
    -- Read coins using Overdrive's UI path
    local currentCoins = u65()
    local maxCoin = getMaxCoins()

    -- Reset condition (exact match or >=, but we use >= for safety)
    if autoResetOnMax and currentCoins >= maxCoin then
        ...
        if LocalPlayer.Character then
            ...
            -- Now kill if reset toggle is on (already checked autoResetOnMax)
            local hum = u59(LocalPlayer.Character, "Humanoid")
            if hum then
                hum.Health = 0
                print("Max coins reached. Teleported and killed.")
            end
        end
        cleanupAutofarm()
        return
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
    cleanupAutofarm()
    -- Re-check game/lobby UI path
    checkGameOrLobby()
end)

-- ============================================================
-- Periodic UI path refresh
-- ============================================================
...
Window:Notify({
    ...
    Content = "Loaded! AutoFarm (Overdrive reset) + AntiAFK + AntiFling",
    Duration = 5,
})

print("✅ Bunnyware V5 Lite loaded (AutoFarm + AntiAFK + AntiFling)")