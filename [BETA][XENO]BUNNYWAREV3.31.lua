...
-- Xeno stability patch
if identifyexecutor and string.lower(identifyexecutor() or ""):find("xeno") then
    -- Override task.spawn to use coroutines more carefully
    local oldSpawn = task.spawn
    task.spawn = function(f)
        return oldSpawn(function()
            xpcall(f, function(err)
                warn("Xeno task error:", err)
            end)
        end)
    end
end
...
-- ============================================================
-- COMBAT TAB (with integrated Deluxe fling & safer silent shoot)
-- ============================================================
...
-- NEW TOGGLE: Make all shoots silent (PC) - SAFER IMPLEMENTATION
local silentAllShoots = true  -- default on
...
-- Perform silent shoot function (used by mouse click)
...
-- Safer silent shoot setup function using MouseButton1Down
local function setupSilentShoot()
    ...
    -- Use MouseButton1Down which is more stable than InputBegan
    silentShootConnection = mouse.Button1Down:Connect(function()
        ...
        if mouse.Target then
            ...
            -- Check if clicking on CoreGui or PlayerGui
            ...
            -- Also check if it's a GUI object
            if target:IsA("GuiObject") or target.Parent:IsA("GuiObject") then
                return
            end
        end
        
        -- Use task.defer to prevent blocking and ensure safe execution
        task.defer(function()
            -- Small delay to prevent any weird interactions
            task.wait(0.025)
            performSilentShoot()
        end)
    end)
    
    print("Silent shoot handler setup successfully")
end

-- Initialize the silent shoot handler
task.defer(function()
...
local function ApplyMisc()
    ...
    if flyEnabled and flyBV then
        ...
        if root and hum then
            ...
            if UserInputService:IsKeyDown(Enum.KeyCode.Space) then                flyBV.Velocity = flyBV.Velocity + Vector3.new(0, 30, 0)
            end
        end
    end
end
