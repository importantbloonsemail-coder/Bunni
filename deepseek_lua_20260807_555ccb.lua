...
-- Uses CoinCollectedEvent for reset – simple and reliable.
-- ============================================================
...
local returnLocation = "Map"  -- kept for compatibility but not used in reset
local farmSpeed = 20
...
AutoSection:Paragraph({
    Title = "📌 Reset Method",
    Desc = "Uses CoinCollectedEvent – always accurate. Resets by killing you."
})
...
-- ============================================================
-- CORE ENGINE – MOVEMENT (tween-based, working)
-- ============================================================
...
if CoinCollectedEvent then
    CoinCollectedEvent.OnClientEvent:Connect(function(cointype, current, max)
        ...
        if autoResetOnMax and current >= max then
...
local function u64(p12)
    -- kept for compatibility but not used in reset anymore
    if p12 == "Map" then
        if u48 then
            local v121 = nil
...
Window:Notify({
    ...
    Content = "Loaded! Resets by killing you when full.",
    Duration = 5,
})

print("✅ Bunnyware V5 Lite (CoinCollected reset) loaded.")